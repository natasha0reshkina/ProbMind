        ...
        <section className="panel">
          ...
          {data.topics.map((topic) => (
            <div className="topic-row" key={topic.topicId}>
              <div className="topic-row__meta"><strong>{topic.name}</strong><span>{topic.observationCount} наблюдений</span></div>
          ...
          <div className="misconception-list">
            {data.misconceptions.filter((m) => m.confidence >= .2).map((m) => (
              <Link className="misconception-row" key={m.misconceptionId} to={`/misconceptions/${m.misconceptionId}`}>
                <div><strong>{m.title}</strong><span>{m.evidenceCount} сигналов</span></div>
                <div className="misconception-row__right"><strong>{Math.round(m.confidence * 100)}%</strong><StatusBadge value={m.status} /></div>
              </Link>
            ))}
          </div>
        </section>
